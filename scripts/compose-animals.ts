 ...
 * (2〜4章)が同じプールを奪い合って必要な5面×3章ぶんの多様性を確保できない
 * ことが判明した。上限を4に緩和したところ、同じ探索で5通り以上に増えることを
 * 実測で確認した（4体のヒョウを含む構成なども新たに出現するようになった）。
 * L3以上を要すること自体が対称な種による背理法を必要とするため、上限を
 * 引き上げても唯一解が壊れるリスクは(a)の理由により本質的には変わらない
 * （壊れる場合はcountSolutions!==1でその場で捨てられる）。
 */
const SYMMETRIC_SPECIES: ReadonlySet<Species> = new Set(['lion', 'leopard', 'rhino']);
const MAX_SYMMETRIC_INSTANCES = 4;
