...
const fitCell = (stage: Stage, windowWidth: number, windowHeight: number): number => {
  ...
  // 柵の太さ（左右あわせてセル約0.44個分。board.tsxのfenceThicknessと合わせる）も
  // 横幅に収まるよう見込んでおく。
  let cell = Math.max(MIN_CELL, Math.min(MAX_CELL, Math.floor(availableWidth / (stage.cols + 0.44))));

  while (cell > MIN_CELL) {
    const fenceThickness = Math.round(cell * 0.22);
